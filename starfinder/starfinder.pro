...
PRO star_param, list, SUBSCRIPTS = s, $
    ...
    n = n_elements(s)
    ;if n lt 2 then print,strc(n)+' stars!' ; TJD
...
PRO starfinder, $
    ...
    BACKGROUND = background, BACK_BOX = back_box, DO_NOT_CHANGE_BACKGROUND = do_not_change_background, $
    ...
    if  n_max ne 0  then begin
       ...
       if  n_elements(back_box) ne 0  and  keyword_set(do_not_change_background) eq 0  then $
    ...
    if  n_stars ne 0  then begin
       list_of_stars = sort_list(extract_stars(list_of_stars, n_stars))
       ;print, "STARFINDER", n_elements(list_of_stars), n_stars
       star_param, list_of_stars, n_stars, $
                   x, y, fluxes, correlation, sigma_x, sigma_y, sigma_f
       ;print, "STARFINDER", n_elements(x)
    endif else begin
